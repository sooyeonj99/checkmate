...
export default function AuthPage() {
  ...
  const reason = searchParams.get('reason') ?? (searchParams.get('expired') === '1' ? 'idle' : null)
...
function LoginForm({ onSuccess, onFindId, onForgotPassword }: {
  ...
  return (
    <form className="auth-form" onSubmit={handleSubmit}>
      ...
      <SocialButtons onGoogleSuccess={(token, user) => { login(token, user); onSuccess() }} />
    </form>
  )
}
...
/* ── 소셜 버튼 (공통) ── */
function SocialButtons({ onGoogleSuccess }: { onGoogleSuccess: (token: string, user: any) => void }) {
  const navigate = useNavigate()
  ...
  useEffect(() => {
    ...
    const handleCredential = async (response: { credential: string }) => {
      try {
        ...
        onGoogleSuccess(data.access_token, data.user)
      } catch (e) {
  ...
  }, [gisReady, googleClientId, onGoogleSuccess])
