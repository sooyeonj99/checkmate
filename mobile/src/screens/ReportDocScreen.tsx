...
type RouteParams = { savedId: number; filename: string }

export default function ReportDocScreen() {
  const route = useRoute()
  const navigation = useNavigation()
  const { savedId, filename } = route.params as RouteParams
  ...
  useEffect(() => {
    api.get(`/contracts/saved/${savedId}/report`)
      .then(res => setHtml(res.data))
      .catch(() => setError('리포트를 불러올 수 없습니다.'))
  ...
  return (
    <View style={styles.root}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1}>{filename} — 리포트</Text>
      </View>
      <WebView
        source={{ html: html! }}
